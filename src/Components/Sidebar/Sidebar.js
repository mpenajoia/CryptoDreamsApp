...
const Sidebar = (props) => {
    ...
    const sidebarMap = defaultArray.map((item, index) => <li><Link to={"/" + item.id}><img src={item.image.small} key={index} alt={item.id}/><span className="sidebar-sym">{item.symbol.toUpperCase()}</span></Link></li>)
