...
const Sidebar = (props) => {
    ...
    const sidebarMap = defaultArray.map((item, index) => <li key={index}><Link to={"/" + item.id}><img src={item.image.small} alt={item.id}/><span className="sidebar-sym">{item.symbol.toUpperCase()}</span></Link></li>)
    const addedCoinMap = props.validCrypto.map((item, index) => <li key={index}><Link to={"/" + item.id}><img src={item.image.small} alt={item.id}/><span className="sidebar-sym">{item.symbol.toUpperCase()}</span></Link></li>)
    return (
